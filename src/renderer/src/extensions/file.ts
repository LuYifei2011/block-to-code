...
//import { Order, pythonGenerator } from 'blockly/generators/python';
...
Blockly.defineBlocksWithJsonArray([
  ...
  },
  {
    ...
    "tooltip": "",
    "helpUrl": "",
    "message0": "打开模式 %1 %2 缓冲 %3 %4 编码 %5 %6 报错级别 %7 %8 换行符 %9 %10 传入的file参数类型 %11 %12 自定义开启器 %13 %14",
    "args0": [
      {
        "type": "field_checkbox",
        "name": "mode",
    ...
    updateShape_: function() {
      if (this.mode_ == 'TRUE') {
        if (!this.getInput('mode')){
          this.appendValueInput('mode').setCheck('String').appendField('模式');
        }
      }else{
        if (this.getInput('mode')){
