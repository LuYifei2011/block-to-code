...
function createWindow(filePath: string, width = 900, height = 670): void {
...
app.whenReady().then(() => {
  ...
  ipcMain.on('newWindow', (_, filePath: string, width?: number, height?: number) => {
  ...
  ipcMain.on('readFile', (event, path: string) => {
    event.returnValue = fs.readFileSync(path, 'utf-8')
  })

  ipcMain.handle('writeFile', async (_, path: string, data: string) => {
